    ...
    <v-app id="inspire">
      <v-row align="center">
        ...
        <v-form ref="form" v-model="valid">
          <v-text-field v-model="user.firstName" :rules="nameRules" label="First Name" required></v-text-field>
          ...
          <v-btn :disabled="!valid" color="success" class="mr-4" @click="editCCA()">Submit</v-btn>
  ...
  methods: {
    ...
    editCCA() {
      api.deleteUser(this.user.username).then(response => {
        api
          .createClinicCenterAdminAgain(this.user, "ClinicCenterAdmin")
          .then(response => {
